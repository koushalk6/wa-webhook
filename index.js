...
const app = express();
app.use(express.json({ limit: '10mb' })); // Handle large WhatsApp payloads
...
// Optional: Log missing secrets (do NOT crash in prod)
if (!VERIFY_TOKEN || !GEMINI_API_KEY || !BUSINESS_PHONE) {
  console.warn('Warning: Missing secrets (fallback used):', {
...
async function handleChatbotBySheetAndGemini(changes) {
  ...
  try {
    ...
    await logOutgoingMessage(phone, responseText, 'text');
  } catch (error) {
...
async function logOutgoingMessage(toPhone, messageText, messageType) {
  ...
  const businessNumber = BUSINESS_PHONE || 'unknown';
...
app.get('/', (req, res) => {
  ...
  // Debug logs
  console.log('VERIFY → mode:', mode, 'token:', token, 'expected:', VERIFY_TOKEN);
...
app.post('/', async (req, res) => {
  ...
  try {
    if (data.entry && data.entry.length > 0 && data.entry[0].changes && data.entry[0].changes.length > 0) {
      const changes = data.entry[0].changes[0].value;

      await Promise.all([
        handleChatbotBySheetAndGemini(changes),
        logData('rawData', data, timestamp, 'Raw Data'),
